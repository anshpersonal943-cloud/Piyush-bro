...
function openPhoto(title, caption, src) {
  ...
  // Enable magnifier when a photo is opened
  enableMagnifierOnModal(src);
}
...
function closePhoto() {
  ...
  // disable magnifier when modal closed
  disableMagnifier();
}

// Magnifier implementation
let _magnifier = null;
let _magnifierEnabled = true;
const MAGNIFIER_ZOOM = 2.6;
function enableMagnifierOnModal(imageSrc) {
  ...
  lens.style.backgroundImage = imageSrc ? `url("${imageSrc}")` : modalPhoto.style.backgroundImage || '';
  document.body.appendChild(lens);
  _magnifier = { lens, imageSrc };

  function moveLens(e) {
    const rect = modalPhoto.getBoundingClientRect();
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    const clientY = e.touches ? e.touches[0].clientY : e.clientY;
    // check within modalPhoto
    ...
    const bgSizeX = rect.width * MAGNIFIER_ZOOM;
    const bgSizeY = rect.height * MAGNIFIER_ZOOM;

    // place lens centered at cursor (fixed positioning)
    lens.style.left = `${clientX}px`;
    lens.style.top = `${clientY}px`;
    lens.style.backgroundSize = `${bgSizeX}px ${bgSizeY}px`;
    // position background so that pixel under cursor is centered in lens
    const posX = (x * MAGNIFIER_ZOOM) - (lens.offsetWidth / 2);
    const posY = (y * MAGNIFIER_ZOOM) - (lens.offsetHeight / 2);
    lens.style.backgroundPosition = `-${posX}px -${posY}px`;
  }

  function enterHandler(e) { moveLens(e); }
  function moveHandler(e) { moveLens(e); }
  function leaveHandler() { lens.style.display = 'none'; }

  modalPhoto.addEventListener('mousemove', moveHandler);
  modalPhoto.addEventListener('touchmove', moveHandler, { passive: true });
  modalPhoto.addEventListener('mouseenter', enterHandler);
  modalPhoto.addEventListener('mouseleave', leaveHandler);

  _magnifier.handlers = { moveHandler, enterHandler, leaveHandler };
}

function disableMagnifier() {
  if (!_magnifier) return;
  const { lens, handlers } = _magnifier;
  if (modalPhoto && handlers) {
    modalPhoto.removeEventListener('mousemove', handlers.moveHandler);
    modalPhoto.removeEventListener('touchmove', handlers.moveHandler);
    modalPhoto.removeEventListener('mouseenter', handlers.enterHandler);
    modalPhoto.removeEventListener('mouseleave', handlers.leaveHandler);
  }
  lens.remove();
  _magnifier = null;
}
